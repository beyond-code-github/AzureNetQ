...
using AzureNetQ.Management.Client;
...
namespace AzureNetQ.Tests
{
    ...
    public class ExceptionConditionsTests
    {
        public void Server_goes_away_after_connection_but_before_publish()
        {
            using(var bus = RabbitHutch.CreateBus("host=localhost"))
            {
                ...
                while(true)
                {
                    ...
                    catch (AzureNetQException exception)
                    {
        ...
        /// <summary>
        /// Ping-pong between two AzureNetQ instances. Try stopping and starting RabbitMQ
        ...
        public void Long_running_consumer_survives_broker_restart()
        {
            using (var publishBus = RabbitHutch.CreateBus("host=localhost;timeout=60", register => 
                register.Register<IAzureNetQLogger>(_ => new NullLogger())))
