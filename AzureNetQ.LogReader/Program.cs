...
namespace AzureNetQ.LogReader
{
    public class Program
    {
        private const string amqpLogExchange = "amq.rabbitmq.log";
        private const string logReaderQueue = "AzureNetQ_LogReader";

        static void Main(string[] args)
        {
            if (args.Length < 1 || args.Length > 2)
            {
                Console.WriteLine("AzureNetQ.LogReader");
                Console.WriteLine("Usage:");
                Console.WriteLine("AzureNetQ.LogReader.exe <rabbitMqServer> [<vhost>]");
                return;
            }
