...
using AzureNetQ.ConnectionString;

namespace AzureNetQ
{
    /// <summary>
    /// Static methods to create AzureNetQ core APIs.
    /// </summary>
    public static class RabbitHutch
    {
        ...
        /// <summary>
        /// Set the container creation function. This allows you to replace AzureNetQ's default internal
        /// IoC container. Note that all components should be registered as singletons. AzureNetQ will
        ...
        /// The AzureNetQ connection string. Example:
        ...
        /// Override default services. For example, to override the default IAzureNetQLogger:
        /// RabbitHutch.CreateBus("host=localhost", x => x.Register&lt;IAzureNetQLogger&gt;(_ => myLogger));
        /// </param>
        /// <returns>
        /// A new RabbitBus instance.
        /// </returns>
        ...
        public static IBus CreateBus(IConnectionConfiguration connectionConfiguration, Action<IServiceRegister> registerServices)
        {
            ...
            if (container == null)
            {
                throw new AzureNetQException("Could not create container. " + 
